...
async function fetchCEOMetrics(companyId: string): Promise<CEOMetrics> {
  ...
  const [
    ...
    supabase.from("customers").select("id", { count: "exact", head: true }).eq("status", "active").eq("company_id", companyId),
    supabase.from("profiles").select("id", { count: "exact", head: true }).eq("company_id", companyId),
    supabase.from("time_clock_entries").select("profile_id").gte("clock_in", todayStart),
    supabase.from("inventory_lots").select("qty_on_hand").gt("qty_on_hand", 0).eq("company_id", companyId),
    supabase.from("communications").select("id", { count: "exact", head: true }).gte("created_at", new Date(Date.now() - 86400000).toISOString()).eq("company_id", companyId),
    supabase.from("social_posts").select("status"),
    supabase.from("accounting_mirror").select("balance, entity_type, data").eq("entity_type", "Invoice").eq("company_id", companyId),
    supabase.from("machine_runs").select("id, status, started_at, ended_at").gte("started_at", todayStart).eq("company_id", companyId),
    supabase.from("cut_plans").select("id", { count: "exact", head: true }).in("status", ["queued", "running"]).eq("company_id", companyId),
    supabase.from("pickup_orders").select("id", { count: "exact", head: true }).eq("status", "ready").eq("company_id", companyId),
    ...
    supabase.from("leads").select("stage, expected_value").eq("company_id", companyId),
    supabase.from("machine_runs").select("started_at, status").gte("started_at", weekAgo).eq("company_id", companyId),
