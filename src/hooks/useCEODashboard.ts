...
async function fetchCEOMetrics(): Promise<CEOMetrics> {
  ...
  const [
    ...
    supabase.from("time_clock_entries").select("profile_id").gte("clock_in", new Date(new Date().setHours(0, 0, 0, 0)).toISOString()),
    ...
    supabase.from("accounting_mirror").select("balance, entity_type").eq("entity_type", "Invoice").gt("balance", 0),
    supabase.from("machine_runs").select("id, status, started_at, ended_at").gte("started_at", new Date(new Date().setHours(0, 0, 0, 0)).toISOString()),
    ...
    supabase.from("machine_runs").select("started_at, status").gte("started_at", new Date(Date.now() - 7 * 86400000).toISOString()),
  ]);
  ...
  // Accounting
  const invoices = accountingRes.data || [];
  const outstandingAR = invoices.reduce((s, i) => s + (i.balance || 0), 0);
  ...
  const dailyProduction = Object.entries(dailyMap).map(([date, data]) => ({
    date,
    ...data,
  }));
  ...
  return {
    ...
    totalPieces,
    completedPieces,
    productionProgress: totalPieces > 0 ? Math.round((completedPieces / totalPieces) * 100) : 0,
    machinesRunning,
    ...
    totalTeam: profilesRes.count || 0,
    teamActiveToday: uniqueWorkers.size,
    ...
    pipelineByStage,
    recentOrders,
    machineStatuses: machines.map((m) => ({
      id: m.id,
      name: m.name,
      type: m.type,
      status: m.status,
