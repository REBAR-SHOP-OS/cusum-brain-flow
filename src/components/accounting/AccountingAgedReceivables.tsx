...
export function AccountingAgedReceivables({ data }: Props) {
  const { qbAction } = data;
  ...
  const runReport = useCallback(async () => {
    setLoading(true);
    try {
      const result = await qbAction("get-aged-receivables", { asOfDate });
      const report = result?.report || result;
      setReportData(parseAgingReport(report));
    } catch (err) {
      console.error("Aged Receivables fetch failed:", err);
      setReportData([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Aging buckets for outstanding customer balances</p>
      </div>
        ...
        <Card>
          <CardContent className="p-0">
            <div className="p-4 pb-2 border-b">
              <h3 className="font-bold text-lg">A/R Aging Detail</h3>
            ...
            {reportData.length === 0 ? (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="text-sm font-semibold">Customer / Transaction</TableHead>
