import { ThumbsUp, ThumbsDown, Mail, Send } from "lucide-react";
...
export function ProspectTable({ prospects, onApprove, onReject, onSendIntro, onSendFollowup, groupLabel }: Props) {
  return (
    <div>
      {groupLabel && (
        <div className="flex items-center gap-2 px-1 py-2">
          <h3 className="text-sm font-semibold text-foreground">{groupLabel}</h3>
          <Badge variant="secondary" className="text-[10px] h-5">{prospects.length}</Badge>
        </div>
      )}
      <Table>
        <TableHeader>
