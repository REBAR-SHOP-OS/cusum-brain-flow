...
Deno.serve((req) =>
  handleRequest(req, async (ctx) => {
    ...
    const now = new Date();
    if (now.getUTCDay() !== 5) {
      return { ok: true, message: "Not a working day, skipping alerts" };
    }
  ...
  }, { functionName: "timeclock-alerts", authMode: "none", requireCompany: false, wrapResult: false, internalOnly: false })
);
