...
Deno.serve((req) =>
  handleRequest(req, async (ctx) => {
    const { videoUrl, platform, aspectRatio } = ctx.body;
    if (!videoUrl) {
...
RESPOND WITH ONLY a JSON object:
{
  ...
  "title": "Short title for the post"
    ...
    try {
      ...
      parsed = { caption: content.trim(), hashtags: [], suggestedTime: "10:00", title: "Video Post" };
    }
  ...
  }, { functionName: "video-to-social", requireCompany: false, wrapResult: false })
);
