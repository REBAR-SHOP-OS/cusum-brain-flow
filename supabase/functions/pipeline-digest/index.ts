...
serve((req) =>
  handleRequest(req, async (ctx) => {
    const { company_id, period = "daily" } = ctx.body;
    ...
    const active = allLeads.filter((l: any) => !terminal.has(l.stage));
    const won = allLeads.filter((l: any) => l.stage === "won");
    const lost = allLeads.filter((l: any) => l.stage === "lost" || l.stage === "loss");
    const breached = active.filter((l: any) => l.sla_breached);
    const now = Date.now();
    const stale = active.filter((l: any) => (now - new Date(l.updated_at).getTime()) / 86400000 >= 14);

    const totalValue = active.reduce((s: number, l: any) => s + ((l.expected_value as number) || 0), 0);
    const weightedValue = active.reduce((s: number, l: any) => s + (((l.expected_value as number) || 0) * ((l.probability as number) ?? 50)) / 100, 0);
...
${breached.length > 0 ? `\nBreached leads: ${breached.slice(0, 5).map((l: any) => `${l.title} (${l.stage})`).join(", ")}` : ""}
${stale.length > 0 ? `\nStale leads: ${stale.slice(0, 5).map((l: any) => `${l.title} (${Math.round((now - new Date(l.updated_at).getTime()) / 86400000)}d)`).join(", ")}` : ""}
${recentEscalations?.length ? `\nRecent escalations: ${recentEscalations.slice(0, 5).map((e: any) => `${e.stage} → ${e.escalated_to}`).join(", ")}` : ""}
  ...
  }, { functionName: "pipeline-digest", requireCompany: false, rawResponse: true })
);
