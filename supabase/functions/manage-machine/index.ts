...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      ...
      // ─── Supervisor override: force-unlock a stuck machine ─────────
      case "supervisor-unlock": {
        // Only admin or shop_supervisor may force-unlock
        ...
        await logProductionEvent(supabaseService, machine.company_id, "supervisor_override", {
          machineId, machineName: machine.name,
          previousRunId: machine.current_run_id,
          previousJobId: machine.active_job_id,
          previousLock: machine.machine_lock,
          reason,
        ...
        events.push({
          ...
          metadata: {
            machineId, machineName: machine.name, reason,
            previousRunId: machine.current_run_id,
            previousJobId: machine.active_job_id,
          },
        });
