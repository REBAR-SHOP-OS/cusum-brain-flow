-- Mirror Detailed List edits (cut_plan_items) back to extract_rows so Tags & Export
-- shows the latest numbers even after a project moves into workstation.
...
BEGIN
  -- Resolve session_id via cut_plans → barlists → extract_session_id
  ...
  -- Match by session + previous mark + previous bar size (so a rename still finds the origin row)
  ...
  -- Track which fields actually changed (for audit payload)
  IF NEW.mark_number     IS DISTINCT FROM OLD.mark_number     THEN v_fields := v_fields || 'mark'; END IF;
  IF NEW.total_pieces    IS DISTINCT FROM OLD.total_pieces    THEN v_fields := v_fields || 'quantity'; END IF;
  IF NEW.cut_length_mm   IS DISTINCT FROM OLD.cut_length_mm   THEN v_fields := v_fields || 'total_length_mm'; END IF;
  IF NEW.bar_code        IS DISTINCT FROM OLD.bar_code        THEN v_fields := v_fields || 'bar_size'; END IF;
  IF NEW.asa_shape_code  IS DISTINCT FROM OLD.asa_shape_code  THEN v_fields := v_fields || 'shape_code'; END IF;
  IF NEW.drawing_ref     IS DISTINCT FROM OLD.drawing_ref     THEN v_fields := v_fields || 'dwg'; END IF;
  IF NEW.bend_dimensions IS DISTINCT FROM OLD.bend_dimensions THEN v_fields := v_fields || 'dims'; END IF;
